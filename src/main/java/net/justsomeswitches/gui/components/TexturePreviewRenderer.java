...
public class TexturePreviewRenderer {
    ...
    /** Updates the font reference after Screen.init() completes. */
     ...
     * Uses putBulkData for correct vertex format handling across render types.
     */
    ...
    /**
     * Gets the current block state for 3D preview, detecting the actual switch type from the world.
     */
    @Nonnull
    private BlockState getCurrentBlockState() {
        Block switchBlock = net.justsomeswitches.init.JustSomeSwitchesModBlocks.SWITCHES_LEVER.get();
        ...
        if (menu.getBlockPos() != null && menu.getLevel() != null) {
            try {
                ...
                if (worldBlock instanceof ISwitchBlock) {
                    switchBlock = worldBlock;
                    isPowered = worldState.getValue(
                            net.minecraft.world.level.block.state.properties.BlockStateProperties.POWERED);
                }
            } catch (Exception e) {
                // Fall back to lever defaults
            }
        }
        return switchBlock.defaultBlockState()
     ...
     * Uses putBulkData for correct vertex format handling with RenderType.solid().
     */
    private void renderBlockWithCustomTextures(@Nonnull BlockState blockState,
        ...
        // Render quads for each face direction and general quads using putBulkData
        for (Direction face : Direction.values()) {
            List<BakedQuad> quads = baseModel.getQuads(blockState, face, random, modelData, RenderType.solid());
            renderQuadsWithShading(quads, poseStack, vertexConsumer, packedLight, packedOverlay);
        }
        List<BakedQuad> generalQuads = baseModel.getQuads(blockState, null, random, modelData, RenderType.solid());
        renderQuadsWithShading(generalQuads, poseStack, vertexConsumer, packedLight, packedOverlay);
    }
    /**
     * Renders quads using putBulkData with face-based brightness shading.
     * putBulkData handles vertex format conversion correctly for any render type.
     */
    private void renderQuadsWithShading(@Nonnull List<BakedQuad> quads, @Nonnull PoseStack poseStack,
    ...
    /**
     * Renders a basic 3D switch preview as fallback using the correct item type.
     */
    private void renderBasic3DPreview(@Nonnull GuiGraphics graphics, int centerX, int centerY) {
        try {
            ItemStack switchStack = getSwitchItemStack();
    ...
    /** Returns the correct ItemStack for the current switch type. */
    @Nonnull
    private ItemStack getSwitchItemStack() {
        if (menu.getBlockPos() != null && menu.getLevel() != null) {
            try {
                BlockState worldState = menu.getLevel().getBlockState(menu.getBlockPos());
                Block worldBlock = worldState.getBlock();
                if (worldBlock instanceof ISwitchBlock) {
                    return new ItemStack(worldBlock.asItem());
                }
            } catch (Exception e) {
                // Fall through to default
            }
        }
        return new ItemStack(net.justsomeswitches.init.JustSomeSwitchesModBlocks.SWITCHES_LEVER_ITEM.get());
    }
    /**
