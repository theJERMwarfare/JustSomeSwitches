...
public class CopyPasteService {
    ...
    public static void copySettingsToWrench(@Nonnull ItemStack stack, @Nonnull SwitchesLeverBlockEntity blockEntity) {
        ...
        NBTHelper.batchNBTOperations(stack, 
            tag -> {
                ...
                // Store GUI items for texture blocks
    ...
    public static void copySelectedSettings(@Nonnull ItemStack stack, @Nonnull SwitchesLeverBlockEntity blockEntity,
        ...
        // Batch all NBT operations for efficiency
        NBTHelper.batchNBTOperations(stack, tag -> {
            // Store settings based on selections
            if (copyToggleFace) {
                settingsTag.putString(WrenchConstants.TOGGLE_FACE_KEY, blockEntity.getToggleTextureVariable());
            }
            if (copyBaseFace) {
            ...
            // Store actual blocks for inventory-based pasting
    ...
    private static boolean compareItemSetting(@Nonnull CompoundTag settingsTag, @Nonnull String key, @Nonnull ItemStack currentItem) {
        if (settingsTag.contains(key)) {
            ItemStack storedItem = ItemStack.of(settingsTag.getCompound(key));
    ...
    public static List<String> validateRequiredBlocks(@Nonnull ItemStack stack, @Nonnull Player player) {
        ...
        // Single inventory check for all required items
        ItemStack[] itemsArray = requiredItems.toArray(new ItemStack[0]);
        if (InventoryHelper.hasAllItems(player, itemsArray)) {
            return new ArrayList<>(); // All items available
        }
        
        // Build missing items list efficiently
    ...
    public static PasteResult applyPartialSettingsFromWrench(@Nonnull ItemStack stack, @Nonnull SwitchesLeverBlockEntity blockEntity, @Nonnull Player player) {
        ...
        // Apply power mode first (always works - no inventory required)
    ...
    private static void applyAllSettings(@Nonnull CompoundTag settingsTag, @Nonnull SwitchesLeverBlockEntity blockEntity, @Nonnull Player player) {
        ...
        applyToggleSettings(settingsTag, blockEntity);
