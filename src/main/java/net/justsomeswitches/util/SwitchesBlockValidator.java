 ...
 * Emissive blocks (Sea Lantern, Glowstone, Magma Block) allowed - texture applied at switch brightness only.
 */
public class SwitchesBlockValidator {
    ...
    /** Validates ItemStack for texture customization (priority: blacklist → whitelist → standard tags → properties). */
    ...
    private static boolean validateBlockProperties(@Nonnull BlockState state) {
        ...
        return SwitchesServerConfig.ALLOW_BLOCK_ENTITIES.get() || !(state.getBlock() instanceof EntityBlock);
    }
}
