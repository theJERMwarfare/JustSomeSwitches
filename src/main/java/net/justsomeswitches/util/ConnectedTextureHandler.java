 ...
 * Extracts standalone base texture (tile 0) from 47-texture CTM systems for ~15-20% of modded blocks.
 */
        ...
        if (isConnectedTextureModLoaded()) {
            try {
                ...
                try (Reader reader = new InputStreamReader(resource.open())) {
                    ...
                    LOGGER.debug("Failed to read CTM metadata for {}: {}", texturePath, e.getMessage());
                }
            } catch (Exception e) {
                LOGGER.debug("Failed to load CTM resource for {}: {}", texturePath, e.getMessage());
            }
        }
    ...
    private static Optional<String> tryCommonBaseTextureNames(@Nonnull ResourceLocation texturePath) {
        ...
        for (String suffix : commonSuffixes) {
            ...
            try {
                ...
                LOGGER.debug("CTM base texture not found at {}: {}", candidatePath, e.getMessage());
            }
        }
        return Optional.empty();
    }
